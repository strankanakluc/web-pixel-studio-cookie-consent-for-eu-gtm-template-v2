...
class CCWPS_Ajax {
	...
	public function request_cookie_preset(): void {
		...
		if ( '' !== $honeypot ) {
			wp_send_json_error( __( 'Nepodarilo sa odoslať žiadosť. Skúste to znova.', 'web-pixel-studio-cookie-consent-for-eu' ) );
		}
		...
		if ( ! is_email( $email ) ) {
			wp_send_json_error( __( 'Zadajte platný e-mail.', 'web-pixel-studio-cookie-consent-for-eu' ) );
		}

		$subject_length = function_exists( 'mb_strlen' ) ? mb_strlen( $subject ) : strlen( $subject );
		if ( $subject_length < 3 || $subject_length > 150 ) {
			wp_send_json_error( __( 'Predmet musí mať 3 až 150 znakov.', 'web-pixel-studio-cookie-consent-for-eu' ) );
		}

		$message_length = function_exists( 'mb_strlen' ) ? mb_strlen( $message ) : strlen( $message );
		if ( $message_length < 20 || $message_length > 4000 ) {
			wp_send_json_error( __( 'Text správy musí mať 20 až 4000 znakov.', 'web-pixel-studio-cookie-consent-for-eu' ) );
		}
		...
		if ( $cooldown > 0 ) {
			wp_send_json_error( __( 'Počkajte prosím 30 sekúnd a skúste to znova.', 'web-pixel-studio-cookie-consent-for-eu' ) );
		}

		if ( $short_count >= 3 ) {
			wp_send_json_error( __( 'Dosiahli ste limit 3 žiadostí za 10 minút. Skúste to neskôr.', 'web-pixel-studio-cookie-consent-for-eu' ) );
		}

		if ( $daily_count >= 10 ) {
			wp_send_json_error( __( 'Denný limit žiadostí bol dosiahnutý. Skúste to zajtra.', 'web-pixel-studio-cookie-consent-for-eu' ) );
		}
		...
		if ( ! $sent ) {
			...
			$error_message = __( 'Žiadosť sa nepodarilo odoslať z tohto WordPress webu. Skontrolujte SMTP/mail konfiguráciu hostingu.', 'web-pixel-studio-cookie-consent-for-eu' );
			if ( '' !== $this->last_mail_error ) {
				$error_message .= ' ' . sprintf(
					/* translators: %s = mail transport error */
						__( 'Detail chyby: %s', 'web-pixel-studio-cookie-consent-for-eu' ),
					$this->last_mail_error
				);
			}
		...
		wp_send_json_success( __( 'Žiadosť bola úspešne odoslaná.', 'web-pixel-studio-cookie-consent-for-eu' ) );
	}
